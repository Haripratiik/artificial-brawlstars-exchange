...
loses eleven significant digits to ``float()``.
...
def test_an_average_price_keeps_every_digit_the_venue_published():
    ...
    to be. This one is twenty-eight significant digits; ``float()`` keeps
    seventeen. The eleven it drops are not noise -- they are what makes
