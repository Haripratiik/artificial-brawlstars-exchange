...
def _runner() -> Any:
    if _RUNNER is None:
        raise _refuse(
            "invalid_request",
            "this exchange is not accepting API traffic yet",
        )
    return _RUNNER
...
def _reconcile(token: str, working: list[dict[str, Any]]) -> None:
    """Tie each working order back to the ``client_order_id`` that placed it.
    ...
    A record that never finds a working order was refused by the venue, filled
    outright, or is still in flight. Those are distinguishable: a refusal shows
    up in ``GET /v1/account/fills`` under ``rejections``, and a fill under
    ``fills``.
    """
    table = _client_orders(token)
    ...
    pending = [
        ...
        if record.order_id is None and record.can_rest
    ]
    if not pending:
        return
            ...
            rows.append(
                {
                    ...
                    "symbol": symbol,
                    "side": who.order_side[(symbol, order_id)].value
                    if (symbol, order_id) in who.order_side
                    else None,
                    "price": None,
                    "ticks": None,
                    "quantity": None,
    ...
    return {
        ...
        "session": {
            ...
            "halted_symbols": sorted(
                s for s in venue.registry.symbols if venue.session(s).value == "halted"
            ),
...
async def instrument(symbol: str) -> dict[str, Any]:
    ...
    indicative = venue.indicative(symbol)
    row["indicative"] = (
        None
...
async def fills(request: Request, limit: str | None = None) -> dict[str, Any]:
    ...
    executions = [entry for entry in log if entry.get("type") == "fill"][-page:][::-1]
    refusals = [entry for entry in log if entry.get("type") == "reject"][-page:][::-1]
...
async def working_orders(request: Request, limit: str | None = None) -> dict[str, Any]:
    ...
    rows = _working_orders(caller)
    _reconcile(caller.token, [row for row in rows if row.get("ticks") is not None])
    for row in rows:
...
def _order_request(payload: dict[str, Any]) -> tuple[dict[str, Any], Any, bool]:
    ...
    # Only an order that can rest is worth remembering for reconciliation. A
    # market order, an immediate-or-cancel and a fill-or-kill never appear in a
    # working-order list by construction, so a pending record for one would
    # never be claimed and would sit in the table until it was evicted.
    can_rest = price is not None and tif not in (
...
async def place_order(request: Request) -> JSONResponse:
    ...
    caller = _authenticate(request, raw)
    order, _listing, can_rest = _order_request(payload)
    ...
    if client_order_id is not None:
        _remember(
            ...
            _ClientOrder(
                ...
                else int(_listing.to_ticks(order["price"])),
                can_rest=can_rest,
    ...
    return JSONResponse(
        {
            ...
            "note": (
                ...
                "exchange's order id appears in GET /v1/orders"
            ),
        },
        status_code=202,
    )
...
async def cancel_all(request: Request) -> dict[str, Any]:
    ...
    caller = _authenticate(request, raw)
    _throttle(caller.token, reducing=True)
    working = _working_orders(caller)
...
async def list_keys(request: Request) -> dict[str, Any]:
    ...
    payload = _paged(rows, len(rows), len(rows), "keys")
