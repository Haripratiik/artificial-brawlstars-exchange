...
inside the generation that issued it, so what is held here is a stable seat
*name* and the account is re-resolved through ``runner.market.seat(name)``
...
def configure(
    *,
    ...
    """
    global _KEYS, _RUNNER, _PATH, _TICK
    if keys is not None:
        _KEYS = keys
...
# Seats by key id, shared across every connection. A credential names an
# account, so two streams opened with one key must read one blotter, the way
# two connections to any real venue do. Per-connection seats would have handed
# the second connection its own account at the first rebuild.
...
def _seat_for(runner: Any, key: Any) -> _Seat:
    ...
    with _SEAT_LOCK:
        ...
        seat = _SEATS.get(key.key_id)

        if (
            seat is not None
            and seat.generation == runner.generation
            and seat.agent_id in market.traders
        ):
            return seat

        if seat is None:
            existing = _adoptable(runner, key)
            if existing is not None:
                # The key names a seat this market has heard of, and was issued
                # after this market started, so the id still means what it
                # meant when it was minted. Following it is what keeps a stream
                # on the same account the signed REST half trades.
                seat = _Seat(
                    str(existing.display_name), existing.agent_id, runner.generation
                )
                _SEATS[key.key_id] = seat
                return seat
            # The label if the key has one, since that is what its owner called
            # it, and the key's account id otherwise -- as a *name*, never as
            # an id to look anybody up by.
            seat = _Seat(str(getattr(key, "label", "") or key.agent_id), None, -1)
            _SEATS[key.key_id] = seat
...
def _adoptable(runner: Any, key: Any) -> Any | None:
    """The live agent a key's id refers to, if the id still means that.

    An account id is only meaningful inside the generation that issued it. Take
    a key issued for ``you-1``, rebuild the market, let anyone else seat first:
    they are now ``you-1``, and a stream that resolved the key by id alone
    would deliver that stranger's fills under this key's name. It is the same
    collision ``dashboard.server._Seat`` records between two browser visitors,
    and it is silent -- both accounts exist, both have a blotter, nothing
    raises.

    So the id is trusted only when the key was minted after the running market
    started. ``MarketRunner.started_at`` is reset by every ``reconfigure``, so
    that comparison answers exactly the question that matters: was this
    credential issued against *this* market. A key with no issue time -- the
    dataclass default is 0.0 -- is never adopted, which is the safe direction:
    a fresh seat is always somebody's own, whereas a wrongly adopted one is
    somebody else's.
    """
    created = float(getattr(key, "created_at", 0.0) or 0.0)
    started = float(getattr(runner, "started_at", 0.0) or 0.0)
    if created <= started:
        return None
    return runner.market.traders.get(AgentId(str(key.agent_id)))
...
class _Stream:
    ...
    async def _refuse(self, code: str, channel: str | None = None, **detail: Any) -> None:
        await self._send(_error_frame(code, channel, **detail))
        ...
        while not self._closed:
            ...
            try:
                if op == "subscribe":
                    ...
                    await self._refuse("invalid_request", op=op)
            except _BadChannel as bad:
                await self._refuse(bad.code, bad.channel or None, **bad.detail)
    ...
    async def _auth(self, message: dict[str, Any], reply_id: Any) -> None:
        ...
        if self._keys is None:
            await self._refuse("auth_invalid")
            return
        try:
        ...
        self._key = key
        self._seat = _seat_for(self._runner, key)
        # A different seat has a different blotter, so the cursor into the old
        # one means nothing. Starting fresh is also what stops a re-auth from
        # replaying the new seat's history.
        self._private_started = False
    ...
    def _collect(self) -> list[dict[str, Any]]:
        ...
        if self._key is not None:
            ...
            self._seat = _seat_for(self._runner, self._key)
