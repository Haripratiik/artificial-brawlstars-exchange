...
```
22,000 lines of exchange, agents and research     745 tests
12,000 lines of tests                             28 listed instruments
...
Correctness in a simulated market is not a matter of opinion, so it is checked mechanically rather than by inspection. This is where most of the work went, and it is what makes the numbers above worth reading.

- **Determinism is a test, not an aspiration.** The same seed and the same manifest produce a bit-identical result digest, down to the byte. That is what turns a paired ablation into a comparison rather than a pair of anecdotes.
- **Conservation is an invariant, not a check at the end.** `conservation_check()` runs after every trade, fee, auction and settlement, in every test and every trial, and returns integer zero. Cash is integer minor units, so there is no tolerance to tune and no drift to explain away.
...
- **The settlement boundary is proved rather than asserted.** Settlement closes cash and positions both out to zero, and the test asserts that identity rather than a rounding window.
- **The front end is part of the test surface.** Two browsers are two traders, and the dashboard drives the same order path the agents use. Unit and display errors that survive every unit test do not survive being read off a screen.
- **Hypotheses are pre-committed.** The information-aggregation result was written into the plan before the run, so the reported comparison is the one that was specified rather than the one that looked best afterwards.

Market data is conflated rather than replayed event by event, which is what keeps a suite that simulates full trading sessions runnable on every change.
...
These are the rules the code was held to, and holding them is most of why the measurements mean anything.
...
- **Determinism is a test.** The same seed and the same manifest give a bit-identical result digest.
- **Hypotheses are pre-committed.** The comparison is written into the plan before the run, so the reported result is the specified one.

## What is deliberately absent

- **Margin and leverage.** Each replaces an exact subtraction with an estimate of what a position is worth before it settles. That trade is available and has not been taken.
