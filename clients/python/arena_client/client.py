...
digits, because average cost is a ratio of two exact integers. ``float()`` on
that string keeps seventeen and discards the rest. So JSON numbers are parsed
...
class ArenaClient:
    ...
    def _request(
        ...
        *,
        ...
        if signed:
            ...
            assert self.secret is not None  # narrowed by `authenticated`
            headers[HEADER_KEY] = str(self.key_id)
            headers[HEADER_TIMESTAMP] = timestamp
            headers[HEADER_SIGNATURE] = sign(
                self.secret, method, signed_path, timestamp, raw
            )
    ...
    def stream_auth(self) -> dict[str, Any]:
        ...
        return {
            ...
            "key_id": self.key_id,
            "timestamp": timestamp,
            "signature": sign(self.secret, "GET", path, timestamp, b""),
        }
